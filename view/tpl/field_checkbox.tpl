	<div id="{{$field.0}}_container" class="clearfix onoffswitch checkbox mb-3">
		<label for="id_{{$field.0}}">{{$field.1}}{{if $field.6}}<sup class="required zuiqmid"> {{$field.6}}</sup>{{/if}}
		</label>
		<div class="float-end">
			...
			<label class="switchlabel" for='id_{{$field.0}}'> 
			 <span class="onoffswitch-inner" data-on='{{if $field.4}}{{$field.4.1}}{{/if}}' data-off='{{if $field.4}}{{$field.4.0}}{{/if}}'>
			 </span>
	...
	<div class="mb-3 form-check">
        <input class="form-check-input" type="checkbox" name="{{$field.0}}" id="id_{{$field.0}}" value="1" {{if $field.2}}checked="checked"{{/if}}
		{{if $field.5}}{{$field.5}}{{/if}} />
        <label class="form-check-label" for="id_{{$field.0}}">{{$field.1}}{{if $field.6}}<sup class="required zuiqmid"> {{$field.6}}</sup>{{/if}}</label>
		<span class="onoffswitch-inner" data-on='{{if $field.4}}{{$field.4.1}}{{/if}}' data-off='{{if $field.4}}{{$field.4.0}}{{/if}}'></span>
		<small class="form-text text-muted">{{$field.3}}</small>
    </div>
{{*
